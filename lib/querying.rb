...
def select_value_and_count_of_most_prolific_species
  "SELECT characters.species, COUNT(characters.species) AS value FROM characters GROUP BY species ORDER BY value DESC LIMIT 1;"
end
...
def select_character_names_and_number_of_books_they_are_in
  <<-SQL
    SELECT characters.name, COUNT(character_books.character_id) AS appearance
    FROM characters
    JOIN character_books ON character_books.character_id = characters.id
    GROUP BY characters.name
    ORDER BY appearance DESC, characters.name
  SQL
end
